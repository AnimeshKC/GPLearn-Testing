...
x_train = rng.uniform(-10, 10, 50)
y_train = rng.uniform(0.001, 10, 50)
trainSet = []
for i in range (len(x_train)):
    trainSet.append([x_train[i], y_train[i]])
z_train = (x_train/y_train) * gpn.inv1(y_train)

#Create Testing sample
x_test = rng.uniform(-10, 10, 50)
y_test = rng.uniform(0.001, 10, 50)
testSet = []
for i in range (len(x_test)):
    testSet.append([x_test[i], y_test[i]])
z_test = (x_test/y_test) * gpn.inv1(y_test)
