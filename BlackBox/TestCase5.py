...
x = rng.uniform(0, 5.001, 50)

y = rng.uniform(0, 5.001, 50)

x, y = np.meshgrid(x, y)
z = gpn.sqrt1(x + y)

#Create Training sample
x_train = rng.uniform(0, 5.0001, 50)

y_train = rng.uniform(0, 5.001, 50)

trainSet = []
for i in range (len(x_train)):
    trainSet.append([x_train[i], y_train[i]])
z_train = gpn.sqrt1(x_train + y_train)

#Create Testing sample
x_test = rng.uniform(0, 5.0001, 50)
y_test = rng.uniform(0, 5.0001, 50)
testSet = []
for i in range (len(x_test)):
    testSet.append([x_test[i], y_test[i]])
    
z_test = gpn.sqrt1(x_test + y_test)
